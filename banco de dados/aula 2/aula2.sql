## Criar a segunda tabela de dados para trabalharmos a relação de tabelas e chave estrangeira 
...
## Criação da interligação fisica entre as tabelas chamada de constraint (foreign key) (chave estrangeira)
...
# Testar se inseriu os dados na produtos

select * from produtos

### Subconsultas é quando temos um select deontro de outro select #

# Clausula in e not int

# In esta dentro contem
# Not in não esta nao contem
...
select Nome,(select Nome from cadastro where id_cad = produtos.id_cad) as Compradores # alias ou apelido para a tabela na saida do select
from produtos

# Trabalhando com Select de campos de mais de 1 tabelas 
...
## JOIN junção relacional de 2 ou mais tabelas

select * from produtos
join cadastro on
produtos.id_cad = cadastro.id_cad

select Nome_Cadastro,Nome_Produtos,Valor,CPF 
from produtos
join cadastro on
produtos.id_cad = cadastro.id_cad 
order by Nome_Cadastro;

# Join 3 tabelas exemplos

#select Nome_Cadastro,Nome_Produtos,Valor,CPF,Qtd_Estoque 
#from produtos
#join cadastro on
#produtos.id_cad = cadastro.id_cad 
#join estoque on
# produto.id_prod = estoque.id_prod
#order by Nome_Cadastro;

## Joins utilizados para analise e comparacao

# LEFT JOIN os valores da esquerda para a direita
...
#############  Criando e alterando registros e tabelas utilizando SELECT ###########

## Criar uma tabela utilizando sub select (copia de uma tabela)
...
## Copiar os registro de uma tabela pra outra
